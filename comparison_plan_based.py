...
def _encode_labels(real_y: pd.Series, syn_y: pd.Series) -> tuple[pd.Series, pd.Series]:
	r_clean = _clean_labels(real_y)
	s_clean = _clean_labels(syn_y)

	joint = pd.concat([r_clean.dropna(), s_clean.dropna()], axis=0).astype(str)
	if joint.empty:
...
def _multivariate_outlier_summary(real_x: pd.DataFrame, syn_x: pd.DataFrame, random_state: int) -> dict[str, float]:
...
def _pmse_summary(real_x: pd.DataFrame, syn_x: pd.DataFrame, random_state: int) -> dict[str, float]:
	x = pd.concat([real_x, syn_x], axis=0, ignore_index=True)
	y = np.concatenate([np.ones(len(real_x), dtype=int), np.zeros(len(syn_x), dtype=int)])

	if len(np.unique(y)) < 2:
		return {
			"pmse": np.nan,
			"domain_classifier_auroc": np.nan,
			"domain_classifier_logloss": np.nan,
		}
...
def _tstr_summary(
	...
	if len(np.unique(y_train)) < 2:
		return {
			...
			"tstr_f1_macro": np.nan,
			"tstr_f1_weighted": np.nan,
			"tstr_auroc": np.nan,
	...
	model = make_pipeline(
		StandardScaler(),
		LogisticRegression(max_iter=2000, solver="lbfgs", class_weight="balanced", random_state=random_state),
	)
	...
	try:
		...
		if len(train_classes) == 2 and len(np.unique(y_eval)) == 2:
			...
			y_bin = (y_eval == pos_class).astype(int)
			auroc = float(roc_auc_score(y_bin, proba_eval[:, pos_idx]))
	...
	return {
		"tstr_accuracy": acc,
		"tstr_f1_macro": f1_macro,
		"tstr_f1_weighted": f1_weighted,
		"tstr_auroc": auroc,
...
def _outlier_specific_utility(real_x: pd.DataFrame, syn_x: pd.DataFrame, random_state: int) -> dict[str, float]:
	...
	ref_scores = -ref.decision_function(real_x)
	ref_auprc = float(average_precision_score(y_true, ref_scores))
...
def run_evaluation(config: EvalConfig) -> None:
	...
	pmse_metrics = _pmse_summary(real_x, syn_x, random_state=config.random_state)
	tstr_metrics = _tstr_summary(real_x, syn_x, real_y_enc, syn_y_enc, random_state=config.random_state)
	outlier_metrics = _outlier_specific_utility(real_x, syn_x, random_state=config.random_state)
	...
	summary = {
		...
		"ks_pvalue_mean": float(np.nanmean(ks_tbl["p_value"].to_numpy())),
	}
	summary.update(_multivariate_outlier_summary(real_x, syn_x, random_state=config.random_state))
	summary.update(_nearest_neighbor_summary(real_x, syn_x))
	summary.update(_validity_summary(real_x, syn_x))
	summary.update(pmse_metrics)
	summary.update(tstr_metrics)
	summary.update(outlier_metrics)
	...
	ks_tbl.to_csv(config.output_dir / "ks_feature_comparison.csv", index=False)
	pd.DataFrame([pmse_metrics]).to_csv(config.output_dir / "pmse_metrics.csv", index=False)
	pd.DataFrame([tstr_metrics]).to_csv(config.output_dir / "tstr_metrics.csv", index=False)
	pd.DataFrame([outlier_metrics]).to_csv(config.output_dir / "outlier_utility_metrics.csv", index=False)
	...
	print(config.output_dir / "ks_feature_comparison.csv")
	print(config.output_dir / "pmse_metrics.csv")
	print(config.output_dir / "tstr_metrics.csv")
	print(config.output_dir / "outlier_utility_metrics.csv")


def parse_args() -> EvalConfig:
	parser = argparse.ArgumentParser(
		description=(
			"Compare original vs synthetic tabular data using quality, resemblance, usability, and outlier-specific metrics."
		)
	)
	parser.add_argument(
		"--dataset",
	...
	parser.add_argument("--random-state", type=int, default=42, help="Random seed for all randomized components")
	...
	dataset_defaults = {
		"heart_disease": {
			...
			# "synthetic": Path("models\\tabsyn-main\\synthetic\\heart_disease\\tabsyn.csv"),
   			# "synthetic": Path("C:\\Users\\melin\\Documents\\Thesis\\Code-final\\BA-Thesis\\models\\TabDiff-main\\tabdiff\\result\\heart_disease\\learnable_schedule\\0\\samples.csv"),
			# "synthetic": Path("C:\\Users\\melin\\Documents\\Thesis\Code-final\\BA-Thesis\\models\\CTGAN\\Fake_Datasets\\synthetic_heart_disease_ctgan.csv"),
   			"synthetic": Path("C:\\Users\\melin\\Documents\\Thesis\Code-final\\BA-Thesis\\models\\CTAB-GAN-Plus\\Fake_Datasets\\Heart_Disease\Heart_Disease_fake_0.csv"),
			"output_dir": Path("comparison_outputs\\heart_disease\\CTABGAN+\\plan_based"),
			"expected_classes": (0, 1),
		}
		# "arrhythmia": {
		# 	"real": Path("Datasets\\Arrhythmia\\arrhythmia.data"),
		# 	"synthetic": Path("models\\CTGAN\\Fake_Datasets\\arrhythmia\\ctgan.csv"),
		# 	"output_dir": Path("comparison_outputs\\arrhythmia\\CTGAN\\plan_based"),
		# 	"expected_classes": tuple(range(1, 17)),
		# },
		# "thyroid": {
		# 	"real": Path("Datasets\\Thyroid\\allbp.test"),
		# 	"synthetic": Path("models\\CTGAN\\Fake_Datasets\\thyroid\\ctgan.csv"),
		# 	"output_dir": Path("comparison_outputs\\thyroid\\CTGAN\\plan_based"),
