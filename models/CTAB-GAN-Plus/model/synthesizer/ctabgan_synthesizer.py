...
class CTABGANSynthesizer:
    def __init__(self,
        ...
        self.tail_penalty_quantiles = tail_penalty_quantiles
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    def _build_tail_penalty_specs(self, train_data):
        low_q, high_q = self.tail_penalty_quantiles
        specs = []
        st = 0

        for column_index, info in enumerate(self.transformer.meta):
            if info["type"] == "continuous":
                column_values = train_data[:, column_index]
                lower = float(np.quantile(column_values, low_q))
                upper = float(np.quantile(column_values, high_q))

                if column_index in self.transformer.general_columns:
                    specs.append({
                        "type": "general",
                        "st": st,
                        "lower": lower,
                    ...
                    component_mask = self.transformer.components[column_index]
                    component_count = int(np.sum(component_mask))
                    means = torch.tensor(
                        self.transformer.model[column_index].means_.reshape(-1)[component_mask],
                        dtype=torch.float32,
                        device=self.device,
                    )
                    stds = torch.tensor(
                        np.sqrt(self.transformer.model[column_index].covariances_).reshape(-1)[component_mask],
                        dtype=torch.float32,
                        device=self.device,
                    )
                    specs.append({
                    ...
                    st += 1 + component_count
            elif info["type"] == "mixed":
                st += 1 + int(np.sum(self.transformer.components[column_index])) + len(info["modal"])
            else:
                st += info["size"]

        return specs

    def _tail_penalty(self, fakeact):
        if self.tail_penalty_lambda <= 0 or not getattr(self, "tail_penalty_specs", None):
            return fakeact.new_tensor(0.0)

        penalties = []

        for spec in self.tail_penalty_specs:
            u = fakeact[:, spec["st"]]

            if spec["type"] == "general":
                reconstructed = ((u + 1.0) / 2.0) * (spec["max"] - spec["min"]) + spec["min"]
            else:
                probs = fakeact[:, spec["st"] + 1: spec["st"] + 1 + spec["component_count"]]
                reconstructed = u * 4.0 * torch.sum(probs * spec["stds"], dim=1) + torch.sum(probs * spec["means"], dim=1)

            penalties.append(F.relu(spec["lower"] - reconstructed) + F.relu(reconstructed - spec["upper"]))
    ...
    def fit(self, train_data=pd.DataFrame, categorical=[], mixed={}, general=[], non_categorical=[], type={}):
        ...
        self.tail_penalty_specs = self._build_tail_penalty_specs(train_data)
        ...
        for i in tqdm(range(self.epochs)):
            for id_ in range(steps_per_epoch):
                ...
                tail_penalty = self._tail_penalty(fakeact)

                _,info_real = discriminator(real_cat_d)
                

                g = -torch.mean(y_fake) + cross_entropy + self.tail_penalty_lambda * tail_penalty
