...
thyroid.py
----------
Train CTAB-GAN+ on the Thyroid dataset and save synthetic samples.

Similar structure to heart_disease.py but adapted for thyroid disease classification.
"""
...
def prepare_thyroid_dataset(
    ...
    The thyroid dataset has: 15 binary symptom columns, 6 continuous test values,
    and 1 label column.
    """
    ...
    if len(df.columns) != len(column_names):
        # Fallback: if columns don't match, just use sequential names
...
def run_experiment(
    ...
    fake_dir = script_dir / "Fake_Datasets" / "Thyroid_epoch20"
    ...
    synthesizer = CTABGAN(
        ...
        integer_columns=CONTINUOUS_COLUMNS,
        problem_type={"Classification": LABEL_COL},
    )
    
    # Pass training hyperparameters to the inner synthesizer
