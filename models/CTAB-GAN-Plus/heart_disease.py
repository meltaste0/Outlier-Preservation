...
# 	fake_dir = script_dir / "Fake_Datasets" / "Heart_Disease_epoch20"
...
# 	)
"""
ctabgan_heart_disease.py
------------------------
Train CTAB-GAN+ on the Heart Disease dataset and save synthetic samples.

Key changes vs original
-----------------------
* Shared preprocessing via preprocessing.py — identical column handling
  across all models.
* fake_paths built from the files generated in this run only (not via
  glob, which would pick up CSVs from previous runs).
* epochs and batch_size are now passed as constructor kwargs via a thin
  subclass so that post-init attribute mutation is no longer needed.
* --max-rows default is consistently 20000 in both run_experiment() and
  argparse (was mismatched in the original).
"""
...
def run_experiment(
    ...
    real_path = script_dir / "Real_Datasets" / "Heart_Disease.csv"
    fake_dir = script_dir / "Fake_Datasets" / "Heart_Disease_Tail_Penalty" / "epoch20"
    fake_dir.mkdir(parents=True, exist_ok=True)
    ...
    prepare_heart_disease(
        ...
        max_rows=max_rows,
        seed=42,
        cast_categoricals_to_str=False,
    )
    ...
    # Pass training hyperparameters directly to the inner synthesizer.
    # NOTE: CTABGAN.__init__ does not expose epochs/batch_size as top-level
    # parameters; these attributes exist on the inner synthesizer object and
    # are read at fit() time, so assigning them here is safe with the current
    # library version. If you upgrade CTABGAN+, verify this still holds.
    synthesizer.synthesizer.epochs = epochs
    synthesizer.synthesizer.batch_size = batch_size
    lam = synthesizer.synthesizer.tail_penalty_lambda

    total_rows = len(pd.read_csv(real_path))
    print(
        f"Config -> rows={total_rows}, epochs={epochs}, "
        f"batch_size={batch_size}, runs={num_exp}, tail_penalty_lambda={lam}"
    )

    # --- Training loop — track only files from THIS run ---
    fake_paths: list[str] = []
    for i in range(num_exp):
        print(f"[CTAB-GAN+] Training run {i + 1}/{num_exp}...")
        synthesizer.fit()
